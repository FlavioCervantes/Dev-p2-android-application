...
public class MainActivity extends AppCompatActivity {
    ...
    private void loginUser(Bundle savedInstanceState){
        ...
        Log.d(TAG, "SharedPref user ID: " + loggedInUserId);
        if(loggedInUserId == LOGGED_OUT & savedInstanceState != null && savedInstanceState.containsKey(SAVED_INSTANCE_STATE_USERID_KEY)){
            loggedInUserId = savedInstanceState.getInt(SAVED_INSTANCE_STATE_USERID_KEY,LOGGED_OUT);
            Log.d(TAG,"Restored user ID from savedInstanceState: " + loggedInUserId);
        }
        if(loggedInUserId == LOGGED_OUT){
            loggedInUserId = getIntent().getIntExtra(MAIN_ACTIVITY_USER_ID,LOGGED_OUT);
            Log.d(TAG,"Fetched user ID from intent: " + loggedInUserId);
        }
        if(loggedInUserId == LOGGED_OUT){
            Log.d(TAG,"No user ID found, launching LoginActivty");
